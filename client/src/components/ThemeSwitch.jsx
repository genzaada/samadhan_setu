...
const ThemeSwitch = () => {
    ...
    const pristineClass = "switch--pristine";

    useEffect(() => {
        // Handle init logic - similar to user's JS
        const el = inputRef.current;
        if (el) {
            // Prevent transitions on mount/init
            el.parentElement?.classList.add(pristineClass);

            // Remove pristine class on first change
            const removePristine = () => {
                el.parentElement?.classList.remove(pristineClass);
            };
            el.addEventListener("change", removePristine, { once: true });

            return () => {
                el.removeEventListener("change", removePristine);
            };
        }
    }, []);
